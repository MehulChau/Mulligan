...
export interface DeviceInfo {
  deviceId: string;
  /** Regenerated by the device on every boot -- absent for a v1.0 device that predates this field. See docs/device-protocol.md's `hello`/`seq` sections. */
  bootId?: string;
...
export class NetworkShotSource extends BaseShotSource {
  ...
  private loggedMissingBootId = false;
  ...
  override async start(): Promise<void> {
    ...
    // A fresh player-initiated session resets dedup tracking outright.
    // Mid-session, a genuine device reboot (detected via a bootId change
    // on a later hello -- see handleHello) resets it the same way; a
    // plain reconnect with the same bootId does not.
    this.lastSeq = -1;
    this.currentBootId = null;
    this.loggedMissingBootId = false;
  ...
  /**
   * Keys dedup on (bootId, seq), not seq alone -- see docs/device-protocol.md's
   * `seq` section. A hello with a bootId different from the one already in
   * use means the device actually rebooted (seq legitimately restarted at
   * 0), so the dedup tracker resets; a hello repeating the same bootId
   * (a plain reconnect) leaves it alone, so a resent shot from before the
   * reconnect is still correctly recognized as a duplicate. A device that
   * never sends bootId (v1.0) falls back to plain seq-only dedup, logged
   * once rather than on every hello.
   */
  private reconcileBootId(bootId: string | undefined): void {
    if (bootId === undefined) {
      if (!this.loggedMissingBootId) {
        this.log("device did not send hello.bootId -- dedup cannot distinguish a reboot from a reconnect for this device");
        this.loggedMissingBootId = true;
      }
      return;
    }
    if (this.currentBootId !== null && bootId !== this.currentBootId) {
      this.log(`device rebooted (bootId changed ${this.currentBootId} -> ${bootId}) -- resetting dedup tracker`);
      this.lastSeq = -1;
    }
